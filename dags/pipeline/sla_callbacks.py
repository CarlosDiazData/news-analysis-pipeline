...
def on_sla_miss(dag, task_list, blocking_task_list, slas, blocking_tis) -> None:
    """Airflow 2.x SLA miss callback — logs one JSON line per miss.
    ...
    Args:
        ...
        slas: List of ``SLAMiss`` namedtuples (dag_id, task_id, execution_date).
    ...
    for miss in slas:
        ...
        sla_exceeded_by = int(
            (now_utc - (miss.execution_date + sla_timedelta)).total_seconds()
        )
        ...
        record = {
            ...
            "scheduled_time": miss.execution_date.isoformat(),
